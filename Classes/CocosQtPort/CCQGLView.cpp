...
static void keyPress(QKeyEvent *event)
{
	s_pMainWindow->keyboardPress(event);
}

static void keyReleased(QKeyEvent *event)
{
	s_pMainWindow->keyboardReleased(event);
}


struct qtKeyCodeItem
{
	int qtKeyCode;
	EventKeyboard::KeyCode keyCode;
};

static std::unordered_map<int, EventKeyboard::KeyCode> g_qtKeyCodeMap;

static qtKeyCodeItem g_qtKeyCodeStructArray[] = {
	/* The unknown key */
	{ Qt::Key_unknown, EventKeyboard::KeyCode::KEY_NONE },
	{ Qt::Key_Up, EventKeyboard::KeyCode::KEY_UP_ARROW },
	{ Qt::Key_Left, EventKeyboard::KeyCode::KEY_LEFT_ARROW },
	{ Qt::Key_Down, EventKeyboard::KeyCode::KEY_DOWN_ARROW },
	{ Qt::Key_Right, EventKeyboard::KeyCode::KEY_RIGHT_ARROW },
	{ Qt::Key_Control,EventKeyboard::KeyCode::KEY_CTRL}
};
...
, _lastWIndowWidth(0.0f)
, _bgColor(0, 0, 0, 1)
{
	g_qtKeyCodeMap.clear();
	for (auto& item : g_qtKeyCodeStructArray)
	{
		g_qtKeyCodeMap[item.qtKeyCode] = item.keyCode;
	}
...
bool CCQGLView::Create()
{
	...
	do
	{
		...
		_window->setKeyEventFunc(&cocos2d::keyPress);
		_window->setKeyReleaseFun(&cocos2d::keyReleased);
...
void CCQGLView::updateFrameSize()
{
	if (_window != nullptr)
	{
		/* resize window if window height changed */
		if (_lastWindowHeight != _window->height() || _lastWIndowWidth != _window->width())
		{
...
void CCQGLView::wheel(QWheelEvent *event)
{
	...
	if (event->orientation() == Qt::Horizontal)
		ccEvent.setScrollData((float)numSteps, 0);
	else
		ccEvent.setScrollData(0,(float)numSteps);
	//ccEvent.setScrollData((float)event->x(), -(float)event->y());
	ccEvent.setCursorPosition(cursorX, cursorY);
	Director::getInstance()->getEventDispatcher()->dispatchEvent(&ccEvent);
}

void CCQGLView::keyboardPress(QKeyEvent *event)
{
	EventKeyboard ccEvent(g_qtKeyCodeMap[event->key()], true);
	Director::getInstance()->getEventDispatcher()->dispatchEvent(&ccEvent);
}

void CCQGLView::keyboardReleased(QKeyEvent *event)
{
	EventKeyboard ccEvent(g_qtKeyCodeMap[event->key()], false);
	Director::getInstance()->getEventDispatcher()->dispatchEvent(&ccEvent);
}
